...
export const HighlightColorButton = () => {
  ...
  const value = editor?.getAttributes("highlight").color || "#FFFF00"; // default yellow
...
const HeadingLevelButton = () => {
  ...
  const headings = [
    ...
    { label: 'Normal text', value: 0, fontSize: '18px' },
  ] as const;
  ...
  return (
    <div className="bg-[#F1F4F9] px-2.5 py-0.5 rounded-[24px] min-h-[40px] flex items-center gap-x-0.5 overflow-x-auto">
      {sections[0].map((item) => (
        ...
        <Separator orientation="vertical" className="mx-2 bg-neutral-400 self-stretch"/>
        <FontFamilyButton />
        <Separator orientation="vertical" className="mx-2 bg-neutral-400 self-stretch" />

        <HeadingLevelButton />
        <Separator orientation="vertical" className="mx-2 bg-black self-stretch" />
        <FontSizeButton />
        <Separator orientation="vertical" className="mx-2 bg-black self-stretch" />
        {sections[1].map((item) => (
            <ToolbarButton key={item.label} {...item} />
