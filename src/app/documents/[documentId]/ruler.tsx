...
// import { editorMargin, editorRulerMarkers, editorRulerMarkersMargin, editorWidth, leftMargin, rightMargin } from '@/config/editor';
...
export const Ruler = () => {
  ...
  // Light casts so TS knows these keys exist in storage
  const leftMarginVal = useStorage((root: any) => (root.leftMargin as number) ?? editorMargin);
  const setLeftMargin = useMutation(({ storage }: { storage: any }, position: number) => {
    (storage as any).set('leftMargin', position);
  }, []);

  const rightMarginVal = useStorage((root: any) => (root.rightMargin as number) ?? editorMargin);
  const setRightMargin = useMutation(({ storage }: { storage: any }, position: number) => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if ((isDraggingLeft || isDraggingRight) && rulerRef.current) {
      ...
      if (container) {
        ...
        if (isDraggingLeft) {
          const maxLeftPosition = editorWidth - rightMarginVal - editorRulerMarkersMargin;
          const newLeftPosition = Math.min(rawPosition, maxLeftPosition);
          setLeftMargin(newLeftPosition);
        } else if (isDraggingRight) {
          const maxRightPosition = editorWidth - (leftMarginVal + editorRulerMarkersMargin);
          const newRightPosition = Math.max(editorWidth - rawPosition, 0);
          const constrainedRightPosition = Math.min(newRightPosition, maxRightPosition);
          setRightMargin(constrainedRightPosition);
        }
  ...
  const handleLeftDoubleClick = () => setLeftMargin(editorMargin);
  const handleRightDoubleClick = () => setRightMargin(editorMargin);
